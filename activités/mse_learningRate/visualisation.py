...
def run_simulation(event):
    ...
    ax.set_title(
        f"secret={secret}  |  lr={lr:.2f}  |  depart={int(depart)}",
        color='white', fontsize=10
    )
    ax.set_xlabel("Etapes", color='#e0e0e0', fontsize=9)
    ...
    document.getElementById("statEtapes").textContent = (
        f"{etapes_convergence}" if etapes_convergence < 30 else "> 30"
    )

# Activer le bouton quand Python est pret
btn = document.getElementById("runBtn")
btn.textContent = "Simuler"
btn.disabled = False
s = document.getElementById("pyscript-status")
s.textContent = "Python pret dans le navigateur !"
